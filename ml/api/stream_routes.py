...
async def video_stream_generator(request: Request, camera_id: str, file_path: str, mode: str, tenant_id: str, analysis_mode: str = "full"):
    """
    Reads video frames from disk, runs ML inference, draws bounding boxes, 
    and yields JPEG bytes for an MJPEG stream.
    """
    ...
    while cap.isOpened():
        ...
        # Control framerate to mimic live playback
        elapsed = time.perf_counter() - start_t
        if elapsed < frame_time:
            # Yield control to event loop
            await asyncio.sleep(frame_time - elapsed)
        else:
            # If we're behind, just yield control briefly
            await asyncio.sleep(0)
